...
> **Примечание:** в современных дистрибутивах (RHEL 9, Debian 12, Ubuntu 22.04+) `iptables` по умолчанию работает поверх `nftables` (вариант `iptables-nft`). Для новых настроек рассмотрите нативный `nftables` — он является официальной заменой `iptables`.
