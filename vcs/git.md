...
*   **Настройка имени основной ветки по умолчанию:**

    ```bash
    git config --global init.defaultBranch main
    ```

    Новые репозитории будут создаваться с веткой `main` вместо устаревшей `master`.
...
*   **Переключение между ветками:**
    ...
    Переключается на указанную ветку. Команда `git switch` (Git 2.23+) — современная замена `git checkout` для работы с ветками.
*   **Создание и переключение на новую ветку:**
    ...
    Создает новую ветку и сразу переключается на нее (аналог устаревшего `git checkout -b`).
*   **Слияние веток:**
...
*   **Отмена изменений в файле:**
    ...
    Возвращает файл к последнему коммиту, отменяя несохраненные изменения. Команда `git restore` (Git 2.23+) — современная замена `git checkout -- <файл>`.
...
*   **Сохранение с сообщением:**
    ...
    Команда `git stash save` объявлена устаревшей (Git 2.16+) — используйте `git stash push -m`.
